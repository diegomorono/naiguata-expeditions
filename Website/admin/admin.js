...
/* ==========================================================================
   CHECKPOINT: PROTECCIÓN DE SESIÓN AL CARGAR LA PÁGINA
   ========================================================================== */
document.addEventListener('DOMContentLoaded', () => {
    const sessionRaw = localStorage.getItem('naiguata_admin_session');

    if (sessionRaw) {
        const session = JSON.parse(sessionRaw);

        // Verificar si la sesión es válida y no ha expirado
        if (session.active === 'true' && Date.now() < session.expiry) {
            // Autenticación válida: Ocultar login, mostrar dashboard
...
document.getElementById('admin-login-form').addEventListener('submit', async (e) => {
    ...
    if (username === 'diegomorono' && typedHash === expectedHash) {
        ...
        const sessionData = {
            active: 'true',
            expiry: Date.now() + (24 * 60 * 60 * 1000) // 24 horas en milisegundos
        };
        localStorage.setItem('naiguata_admin_session', JSON.stringify(sessionData));
        initializeAdminDashboard();
    } else {
...
async function initializeAdminDashboard() {
    ...
    // Agrega esta función para gestionar el toggle
    function setupFinancialFilters() {
        const filterEvent = document.getElementById('fin-filter-event');
        const filterAll = document.getElementById('fin-filter-all');

        if (!filterEvent || !filterAll) return;

        const toggle = (activeBtn, inactiveBtn) => {
            activeBtn.classList.add('active');
            inactiveBtn.classList.remove('active');
            // Aquí refrescamos la vista
            renderFinancials();
        };

        filterEvent.addEventListener('click', () => toggle(filterEvent, filterAll));
        filterAll.addEventListener('click', () => toggle(filterAll, filterEvent));
    }
    ...
    // 1. Forzar Tasa BCV
    const btnBcv = document.getElementById('btn-update-bcv');
    if (btnBcv) {
        btnBcv.addEventListener('click', handleUpdateBCV);
    }

    // 2. Simular Excursionista
    const btnMock = document.getElementById('btn-add-mock-participant');
    if (btnMock) {
        btnMock.addEventListener('click', handleAddMockParticipant);
    }

    // 3. Aplicar Distribución (Ya existe la función, solo asegúrate de llamarla)
    const btnTents = document.getElementById('btn-reallocate-tents');
    if (btnTents) {
        btnTents.addEventListener('click', () => {
            runTentAllocation(); // Esta es la función que ya tenías
            alert('Distribución de carpas actualizada.');
        });
    }
    ...
    async function loadCatalogs() {
        const { data: inv } = await supabaseClient.from('inventory_stock').select('*');
        const { data: cat } = await supabaseClient.from('catering_inventory').select('*');
        const { data: serv } = await supabaseClient.from('logistic_services').select('*');
    ...
    window.cancelRegistration = async function (id) {
        ...
        if (error) {
            ...
            await supabaseClient.from('financial_transactions').delete().eq('registration_id', id);
            updateDashboardData();
        }
    };
    ...
    function runTentAllocation() {
        ...
        if (!container) {
            console.warn("Contenedor de carpas no encontrado.");
            return;
        }
        container.innerHTML = '';

        // Filtrar confirmados y asegurar que tengan campos básicos
        const confirmedHikers = adminState.bookings.filter(b =>
            b.status === '🟢 Confirmado' && b.name && b.gender
        );
        ...
        // Asignar grupos (con estrategia de rentabilidad: si no caben, se dividen)
        Object.keys(groupsMap).forEach(code => {
            const members = groupsMap[code];
            // Intentar buscar carpa para el grupo
            let bestTent = ownedTents.find(t => t.occupants.length === 0 && t.capacity >= members.length) ||
                ownedTents.find(t => (t.capacity - t.occupants.length) >= members.length) ||
                externalTents.find(t => t.occupants.length === 0 && t.capacity >= members.length);
            ...
            // Si no entra, NO hacemos nada. Se quedan en 'unassigned' para el siguiente paso.
        });
        ...
        const assignSoloGender = (solos) => {
            solos.forEach(solo => {
                let target = [...ownedTents, ...externalTents].find(t =>
                    t.occupants.length > 0 && t.occupants.length < t.capacity && t.occupants.every(o => o.gender === solo.gender)
                );

                if (!target) target = ownedTents.find(t => t.occupants.length === 0);
                if (!target) target = externalTents.find(t => t.occupants.length === 0);
                if (!target) target = [...ownedTents, ...externalTents].find(t => t.occupants.length < t.capacity);

                // Paso 5: Emergencia (Forzado)
                if (!target) target = [...ownedTents, ...externalTents].find(t => t.occupants.length < t.capacity);

                if (target) {
                    target.occupants.push(solo);
                    const idx = unassigned.findIndex(u => u.id === solo.id);
                    if (idx !== -1) unassigned.splice(idx, 1);
                }
            });
        };
        ...
        if (unassigned.length > 0) {
            ...
            alertDiv.style.cssText = 'color:var(--error); text-align:center; padding:10px; grid-column:1/-1; border:1px solid var(--error); margin-top:10px;';
            alertDiv.innerHTML = `<strong>⚠️ ALERTA:</strong> Faltan ${unassigned.length} excursionistas por asignar por falta de espacio físico.`;
    ...
    function setupExpenseForm() {
        ...
        document.getElementById('permuta-register-form').addEventListener('submit', async (e) => {
            ...
            const { error: error1 } = await supabaseClient.from('financial_transactions').insert({
                date: adminState.selectedDate,
                type: 'Egreso',
            ...
            const { error: error2 } = await supabaseClient.from('financial_transactions').insert({
                date: adminState.selectedDate,
                type: 'Ingreso',
...
• Bananas (Cambur): ${N} unidades (más 2 de respaldo)`;
    ...
    /* ==========================================================================
       FUNCIONES DE SOPORTE PARA BOTONES (MVP)
       ========================================================================== */

    // 1. Forzar Tasa BCV
    async function handleUpdateBCV() {
        ...
        const { error } = await supabaseClient
            ...
            .eq('key', 'last_valid_bcv'); // Asegúrate que este sea el key en tu BD

        if (error) alert("Error al actualizar tasa: " + error.message);
        else {
            alert("Tasa actualizada exitosamente.");
            loadInitialSettings(); // Recarga los settings
        }
    }

    // 2. Simular Excursionista (Mock Data)
    async function handleAddMockParticipant() {
        const mockData = {
            name: "Excursionista de Prueba",
        ...
        const { error } = await supabaseClient.from('registrations').insert([mockData]);
        if (error) alert("Error al crear participante: " + error.message);
        else updateDashboardData(); // Refresca el dashboard
    }
