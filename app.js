...
// Track the last submitted booking for direct contact sharing
...
function initBookingForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Keep last booking reference for WhatsApp/Email sharing
        lastBooking = booking;
    ...
    // WhatsApp and Email guide buttons listeners
    const btnWhatsappGuide = document.getElementById('btn-whatsapp-guide');
    const btnEmailGuide = document.getElementById('btn-email-guide');

    btnWhatsappGuide.addEventListener('click', () => {
        if (!lastBooking) return;
        
        // Format message
        const message = `¡Hola Diego! Acabo de registrarme para la expedición al Pico Naiguatá ⛰️
Mis datos de reserva:
- Nombre: ${lastBooking.name}
- Email: ${lastBooking.email}
- WhatsApp: ${lastBooking.whatsapp}
- Código de Grupo: ${lastBooking.group || 'Ninguno'}
- Género: ${lastBooking.gender}
- Alojamiento: ${lastBooking.tentPreference === 'couple' ? 'Pareja (Carpa 2P)' : 'Compartido (Carpa 2P/3P)'}
- Dieta: ${lastBooking.diet} / Alergias: ${lastBooking.allergies}
- Médica/Notas: ${lastBooking.medical}
- Alquileres: ${lastBooking.rentals.length > 0 ? lastBooking.rentals.map(r => r === 'sleeping-bag' ? 'Saco' : r === 'sleeping-pad' ? 'Aislante' : 'Bastones').join(', ') : 'Ninguno'}
- Código de Pase: ${lastBooking.id}
- Total a Pagar: $${lastBooking.totalUsd.toFixed(2)} USD (o al cambio oficial BCV: Bs. ${(lastBooking.totalUsd * appState.bcvRate).toFixed(2)})`;

        // Open WhatsApp link
        const waUrl = `https://wa.me/34673375681?text=${encodeURIComponent(message)}`;
        window.open(waUrl, '_blank');
    });

    btnEmailGuide.addEventListener('click', () => {
        if (!lastBooking) return;
        
        const subject = `Nueva Reserva de Expedición Naiguatá - ${lastBooking.name}`;
        const body = `Hola Diego,
        
He registrado mis datos en la web de Naiguatá Expeditions. Aquí tienes los detalles de mi reserva:

- Senderista: ${lastBooking.name}
- Email: ${lastBooking.email}
- WhatsApp: ${lastBooking.whatsapp}
- Grupo: ${lastBooking.group || 'Ninguno'}
- Género: ${lastBooking.gender}
- Preferencia de Carpa: ${lastBooking.tentPreference === 'couple' ? 'Pareja' : 'Compartido'}
- Dieta: ${lastBooking.diet} / Alergias: ${lastBooking.allergies}
- Datos Médicos: ${lastBooking.medical}
- Alquileres: ${lastBooking.rentals.map(r => r === 'sleeping-bag' ? 'Saco' : r === 'sleeping-pad' ? 'Aislante' : 'Bastones').join(', ') || 'Ninguno'}
- Pase de Cumbre ID: ${lastBooking.id}
- Total: $${lastBooking.totalUsd.toFixed(2)} USD (o al cambio oficial BCV: Bs. ${(lastBooking.totalUsd * appState.bcvRate).toFixed(2)})

¡Coordinemos el pago!`;

        const mailtoUrl = `mailto:diego.morono03@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
